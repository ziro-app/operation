...
import { close, container, data, modal } from './styles';
import { animate, initial, transition } from './animation';
        ...
        if (isOpen) {
            ...
            if (arrowDown) {
                setCursorPosition(prevPosition => {
                    if (prevPosition === null) return 0;
                    if (prevPosition < maxPosition) return prevPosition + 1;
                    return prevPosition;
                });
                setArrowDown(false);
            }
            if (arrowUp) {
                setCursorPosition(prevPosition => {
                    if (prevPosition > 0) return prevPosition - 1;
                    return prevPosition;
                });
                setArrowUp(false);
            }
            ...
            {isOpen && (
                <motion.div style={modal} initial={initial} animate={animate} transition={transition}>
                    {options.map((item, index) => (
                        <input style={data(cursorPosition === index)} value={item} onMouseDown={handleSelection}
                               key={item} id={index} readOnly={true}/>
                    ))}
                </motion.div>
            )}
        </div>
