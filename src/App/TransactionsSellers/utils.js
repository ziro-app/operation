...
export const translateProductType = (name) => {
    switch (name) {
        case 'backgroundCredit':
            return "Crédito de Background Check"
    ...
    switch (type) {
      case 'allFilters':
        return db
          ...
          .where('dateLastUpdated', '>=', dataInicio)
          .where('dateLastUpdated', '<=', dataFim)
      case 'month':
        return db
          .collection('payments-sellers-ziro')
          .orderBy('dateLastUpdated', 'desc')
          ...
          .where('dateLastUpdated', '<=', dataFim)
      case 'status':
        return db.collection('payments-sellers-ziro').orderBy('dateLastUpdated', 'desc').where('status', '==', `${storageFilterStatus}`)
      case 'seller':
        return db.collection('payments-sellers-ziro').orderBy('dateLastUpdated', 'desc').where('seller', '==', `${storageFilterSeller}`)
      ...
      case 'status and seller':
        return db
          ...
          .where('status', '==', `${storageFilterStatus}`)
      default:
        return db.collection('payments-sellers-ziro').orderBy('dateLastUpdated', 'desc')
    }
  }
  switch (type) {
    case 'allFilters':
    ...
    case 'month':
      return db
        ...
        .limit(limit)
    case 'status':
      return db.collection('payments-sellers-ziro').orderBy('dateLastUpdated', 'desc').where('status', '==', `${storageFilterStatus}`).limit(limit)
    case 'seller':
      return db.collection('payments-sellers-ziro').orderBy('dateLastUpdated', 'desc').where('seller', '==', `${storageFilterSeller}`).limit(limit)
    case 'month and status':
      return db
        .collection('payments-sellers-ziro')
        .orderBy('dateLastUpdated', 'desc')
    ...
    case 'status and seller':
      return db
        ...
        .limit(limit)
    default:
      return db.collection('payments-sellers-ziro').orderBy('dateLastUpdated', 'desc').limit(limit)
  }
}
