...
import { translateProductType, allowedUsersToUpdateTransactions, dateFormat, parcelFormat, round, stringToFloat } from '../../utils'
...
const TransactionDetails = ({ transactions, transactionId, transaction, setTransaction }) => {
  ...
  let markupTransaction : ISplit
  let antiFraudTransaction : ISplit
  ...
  const cancelTransaction = async (transaction_id,installments, on_behalf_of, amountBeforeConvert, buyerDocId,quantityToRemove) => {
    try {
      if (process.env.HOMOLOG ? true : allowedUsersToUpdateTransactions.includes(nickname)) {
        ...
        const snapRefSuppliers = db.collection('suppliers').doc(buyerDocId)
  ...
  const deleteTransaction = async () => {
    if (process.env.HOMOLOG ? true : allowedUsersToUpdateTransactions.includes(nickname)) {
      ...
      setIsDeleting(true)
      try {
        await db.collection('payments-sellers-ziro').doc(transactionId).delete()
        setLocation('/transacoes-fabricantes')
        setIsLoading(false)
      } catch (error) {
        console.log(error)
        ...
        block = [
          {
            ...
            body: [
              ...
              },
              {
                ...
                content: translateProductType(transaction.productType),
              },
              {
                title: 'Quantidade',
        ...
        if (typeof transaction.receivables !== 'undefined' && transaction.receivables.length) {
          ...
          sortedTransactions.map(transaction => {
            ...
            if (!transaction.paid_at) {
              ...
              unpaidClicks.push(() => setLocation(`/transacoes-fabricantes/${transactionId}/${transaction.receivableZoopId}`))
              ...
              paidClicks.push(() => setLocation(`/transacoes-fabricantes/${transactionId}/${transaction.receivableZoopId}`))
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={containerWithPadding}>
      ...
      <Header type="icon-link" title="Detalhes da venda" navigateTo="transacoes-fabricantes" icon="back" />
