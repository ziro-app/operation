...
const CreatePayment = () => {
  const allowedUsersToTest = ['Uiller', 'Vitor', 'João', 'Ale', 'Antonio','Russi']
  ...
  const [hasBrandZoopPlan, setHasBrandZoopPlan] = useState(null)
  ...
  useEffect(() => {
    if (brand) {
        async function getBrandZoopPlan() {
          const getSupplierData = await db.collection('suppliers').where('fantasia', '==', brand.toUpperCase()).get()
          getSupplierData.forEach(doc => {
            isNewPlan ? setHasBrandZoopPlan(doc.data().sellerZoopPlan2 || null) : setHasBrandZoopPlan(doc.data().sellerZoopPlan || null)
          })
        }
        getBrandZoopPlan()
      }
