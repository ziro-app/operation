...
import { brandName, bubble, card, cart, dot, statusBlock, statusName, timestamp } from './styles';
              ...
              {_carts.byStatus[EN].map(cart => (
                <motion.div key={cart.id} whileTap={{ scale: 0.95 }} onClick={() => setLocation(`/pedidos/${cart.id}`)} style={card}>
                  <div style={{ display: 'grid' }}>
                      ...
                      <label
                          style={timestamp}>{cart.lastUpdate ? `Atualizado ${cart.lastUpdate}` : `Adicionado ${cart.added}`}</label>
                  </div>
