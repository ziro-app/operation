...
import {
    prepareStatusForConsume,
    prepareStoreownersForConsume,
    prepareSellersToConsume,
    prepareCartsForConsume,
} from "../utils";
            ...
            >
                ...
                <Dropdown
                    ...
                    onChange={({ target: { value } }) =>
                        !value || _status.convert.PTtoEN[value]
                            ? setStatus(_status.convert.PTtoEN[value])
                            : setBufferStatus(value)
                    }
                ...
                <Dropdown
                    ...
                    onChange={({ target: { value } }) =>
                        !value || _storeowners.convert.RazaoToId[value]
                            ? setStoreownerId(
                                  _storeowners.convert.RazaoToId[value]
                              )
                            : setBufferRazao(value)
                    }
            ...
            <div style={cart}>
                {_status.options.EN.map((EN) => (
                    <div key={EN} style={statusBlock}>
                        <label style={statusName}>
                            {_status.convert.ENtoPT[EN]}
                        </label>
                        {_carts.byStatus[EN].map((cart) => (
                            ...
                            >
                                <div style={{ display: "grid" }}>
                                    ...
                                    >
                                        {
                                            _storeowners.convert.IdToRazao[
                                                cart.storeownerId
                                            ]
                                        }
                                        <span style={dot}>.</span>
                                    ...
                                    >
                                        {cart.lastUpdate
                                            ? `Atualizado em: ${cart.lastUpdate}`
                                            : `Adcionado em: ${cart.added}`}
                                    </label>
                                </div>
                                <div style={bubble}>
